...
use warnings;
use Test::More;
use Test::Quattor qw(pxelinux_ks_block pxelinux_ks_ramdisk_size);
...
for my $variant_constant (@PXE_VARIANTS) {
    ...
    $fh = get_file($fp);
    like($fh, qr{^\s{4}$kernel_params_cmd.*?\sramdisk_size=123456(\s|$)}m, "append ramdisk (variant=$variant_name)");
};
